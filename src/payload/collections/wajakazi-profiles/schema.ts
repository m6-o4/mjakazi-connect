...
import { ensureSlug } from "@/payload/collections/wajakazi-profiles/hooks/ensure-slug";
import {
	revalidateProfile,
	revalidateProfileDelete,
} from "@/payload/collections/wajakazi-profiles/hooks/revalidate-profile";
...
const WajakaziProfiles: CollectionConfig = {
	...
	fields: [
		...
		},
		{
			...
			label: "Slug",
			index: true,
			// generated by the ensureSlug hook (display name + random suffix) and
			// read-only so no path can change the public URL after creation
			admin: { readOnly: true, position: "sidebar" },
		},
		// --- identity ---
		{
	...
	hooks: {
		...
		afterDelete: [revalidateProfileDelete],
	},
	timestamps: true,
};
