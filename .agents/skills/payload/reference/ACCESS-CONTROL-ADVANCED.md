...
Advanced access control patterns including context-aware access, time-based restrictions,
factory functions, and production templates.
...
**Note**: Requires your server to pass IP address via headers (common with proxies/load
balancers).
...
13. **Handle Errors Gracefully**: Access functions should return `false` on error, not
    throw
