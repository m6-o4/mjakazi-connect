...
> `versions: { drafts: true }` injects a managed `_status` field (`draft` / `published` /
> `changed`) that the admin UI and Draft Preview already understand. Use it in
> `defaultColumns` and access control directly.
