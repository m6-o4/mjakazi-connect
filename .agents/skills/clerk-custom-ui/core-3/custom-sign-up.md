...
Use `update()` to add optional fields (name, metadata, legal acceptance, locale) to an
existing sign-up before finalization.
...
If `signUp.isTransferable` is `true`, the identifier matches an existing user and the
sign-up should be transferred to a sign-in flow. This involves coordinating between
sign-up and sign-in resources. See the
[transferable sign-up docs](https://clerk.com/docs/custom-flows/overview) for the full
implementation.
...
All methods return `Promise<{ error: ClerkError | null }>`. Errors are also available
reactively on the hook:
