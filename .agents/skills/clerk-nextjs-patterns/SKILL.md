...
compatibility:
  Requires NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY and CLERK_SECRET_KEY. For manual JWT
  verification (standalone API servers without Clerk middleware), additionally requires
  CLERK_JWT_KEY or CLERK_PEM_PUBLIC_KEY.
metadata:
  author: clerk
  version: 2.2.0
---
...
> **Core 2 ONLY (skip if current SDK):** `isAuthenticated` and `sessionStatus` are not
> available. Check `!!userId` instead.
...
> **Core 2 ONLY (skip if current SDK):** `<Show>` does not exist. For authentication, use
> `<SignedIn>` and `<SignedOut>`. For authorization (role / permission), use `<Protect>`
> with the same prop names (`role`, `permission`, `condition`). Feature- and plan-based
> variants require Core 3. See `clerk-custom-ui` skill, `core-3/show-component.md` for the
...
Pass a custom JWT to third-party services (Hasura, Supabase, etc.) using JWT templates
defined in the Clerk dashboard.
...
`getToken()` returns `null` when the user is not authenticated — always null-check before
use.
...
For standalone API servers that receive Clerk session tokens from the `Authorization`
header or the `__session` cookie (same-origin).
...
> **CRITICAL**: Always check `exp` and `nbf` claims. `verifyToken` from `@clerk/backend`
> handles this automatically; with raw `jsonwebtoken`, set `ignoreExpiration: false`
> (default) and ensure `clockTolerance` is minimal.
